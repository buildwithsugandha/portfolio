...
const projects = [
  {
    ...
    outcome: "ASG auto-scales from 2→6 EC2 instances under load. Multi-AZ RDS failover completes in under 60 seconds. Terraform apply time reduced from hours of manual work to under 8 minutes.",
  ...
  },
  {
    ...
    outcome: "MTTD reduced from ~15 minutes (manual log review) to under 2 minutes via Alertmanager firing before user impact. Dashboard covers 20+ Kubernetes metrics across pods, nodes, and SLOs.",
  ...
  },
  {
    ...
    outcome: "Eliminated configuration drift across 3 environments (dev/staging/prod). Infra provisioning time cut from 4+ hours of manual clicks to a single `terraform apply` run in under 12 minutes.",
  ...
  },
  {
    ...
    outcome: "Hybrid tunnel established with sub-100ms latency between on-prem and Azure VNet. BGP failover tested at under 30 seconds. Runbooks cut incident resolution time from 45+ minutes to under 10 minutes.",
  ...
  },
  {
    ...
    outcome: "End-to-end pipeline runs in under 6 minutes (lint → test → build → scan → deploy). Trivy blocks HIGH/CRITICAL CVEs before production. Rollback completes in under 2 minutes on failure — zero bad deployments reach users.",
